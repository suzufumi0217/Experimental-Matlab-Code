...
info.ts1 = 25; % Main time in ms %stimulation in 40Hz in this situation   
info.ts2 = 12; % Group time in ms(number of channel*1.5)
...
info = FES_InitializeAmplitude(motionstim8, info);


% Initialize all channels for upcoming stimulation
initializationHexCode = FES_ChannelListInitialization(info.ts1, info.ts2, info.AllChannels, info.lowFreqChannels, info.N);
